...
export default function App() {
  ...
  const handleUpdateUserByAdmin = async (userId, data) => {
    try {
      const userRef = doc(db, "users", userId);
  ...
  // Pastikan fungsi handleDeletePost dan handleDeleteMading juga sudah ada
          ...
          <div className="comic-box bg-white p-6 md:p-8">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              <div className="flex-grow text-center sm:text-left space-y-2">
                {/* ── FITUR EDIT/TAMPILAN NAMA USER ── */}
                <div className="flex flex-wrap justify-center sm:justify-start items-center gap-2 min-h-[36px] group">
                  {isEditingName ? (
                    <div className="flex gap-1.5 items-center w-full max-w-[280px]">
                      <input
                        type="text"
                        value={newName}
                        onChange={(e) => setNewName(e.target.value)}
                        className="comic-textarea text-xs py-1 px-2 w-full font-extrabold"
                        maxLength={30}
                        placeholder="Ketik nama baru..."
                        autoFocus
                      />
                      <button onClick={handleSaveName} className="comic-btn bg-green-400 text-[10px] py-1 px-2 font-bold flex-shrink-0">
                        Ok
                      </button>
                      <button onClick={() => setIsEditingName(false)} className="comic-btn bg-gray-200 text-[10px] py-1 px-2 flex-shrink-0">
                        X
                      </button>
                    </div>
                  ) : (
                    <>
                      ...
                      <button
                        ...
                        className="text-[9px] bg-white border-2 border-black px-1.5 py-0.5 rounded-md font-black shadow-[1px_1px_0px_#000] hover:bg-gray-100 opacity-0 group-hover:opacity-100 transition-opacity ml-1"
